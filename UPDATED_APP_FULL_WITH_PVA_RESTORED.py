...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals", "HubSpot Deal Score tracker", "Marketing Lead Performance & Requirement", "Kids detail", "Deal Detail", "Sales Intern Funnel", "Intern vs AC", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Trend", "Deal Score Threshold", "Invalid Deals", "Super report", "Marketing Plan", "Plan vs Actual"],
    }
...
# =========================
# Cash-in Tab (Google Sheet A1:BB13 main table, A14:BB14 total)
# =========================
elif view == "Cash-in":
    ...
    default_gsheet_url = "https://docs.google.com/spreadsheets/d/1ygmNCuERA2kkWGztQCmX9sTZlyTZcLp_UM0_5GPZxN4/edit?gid=0#gid=0"
    ...
    main_table = df_raw.iloc[0:13, :54].copy()   # rows 0..12 => A1:BB13
    total_df   = df_raw.iloc[13:14, :54].copy()  # row 13       => A14:BB14

    # ---- Show the table (A1..D13) ----
    st.markdown("#### Table (A1:BB13)")
    if main_table.empty:
        st.info("No data rows found in A1:BB13.")
    else:
        st.dataframe(main_table, use_container_width=True)
        st.download_button(
            "Download CSV (table A1:BB13)",
    ...
    st.markdown("#### Total (A14:BB14)")
    if total_df is None or total_df.empty:
        st.info("Total row not found at A14:BB14.")
    else:
        st.dataframe(total_df, use_container_width=True)
        st.download_button(
            "Download CSV (total A14:BB14)",
...
def _render_marketing_sales_intern_funnel(df):
    ...
    # Prefer explicit Sales - Intern column; fallback if missing
    if "Sales - Intern" in df.columns:
        intern_col = "Sales - Intern"
    else:
        intern_col = _col(df, [
...
# -------------------- Marketing • Super report --------------------
def _render_marketing_super_report(df):
    import streamlit as st
    import pandas as pd
    import numpy as np
    import altair as alt

    st.subheader("Super report — Dual window comparison")

    if df is None or getattr(df, "empty", True):
        st.info("No data available.")
        return

    # Use global find_col if available, else local fallback
    try:
        _find_col = find_col
    except NameError:
        def _find_col(_df, _cands):
            for _c in _cands:
                if _c in _df.columns:
                    return _c
            _low = {c.lower(): c for c in _df.columns}
            for _c in _cands:
                if _c.lower() in _low:
                    return _low[_c.lower()]
            return None

    base = df.copy()

    # --- Column resolution ---
    thr_col = _find_col(base, [
        "New Deal Score threshold",
        "New deal score threshold",
        "New Lead Threshold",
        "New lead threshold",
        "New lead threshold variable",
        "New Lead Threshold Variable",
    ])

    create_col = _find_col(base, [
        "Create Date", "Created Date", "Deal Create Date",
        "Created On", "Create_Date",
    ])

    pay_col = _find_col(base, [
        "Payment Received Date", "Payment Date",
        "Enrolment Date", "Enrollment Date",
        "Payment_Received_Date", "Paid On",
    ])

    first_cal_col = _find_col(base, [
        "First Calibration Scheduled Date",
        "First Calibration Date",
        "First_Calibration_Scheduled_Date",
        "First Calibration Scheduled date",
    ])

    cal_resch_col = _find_col(base, [
        "Calibration Rescheduled Date",
        "Calibration_Rescheduled_Date",
        "Calibration reschedule date",
        "Calibration rescheduled date",
    ])

    cal_book_col = _find_col(base, [
        "Calibration Booking Date",
        "Calibration Booked Date",
        "Calibration booking date",
    ])

    done_col = _find_col(base, [
        "Calibration Done Date",
        "Calibration done date",
        "Calibration_Done_Date",
    ])

    ac_col = _find_col(base, [
    ...
    src_col = _find_col(base, [
        "JetLearn Deal Source",
        "Deal Source",
        "Original Deal Source",
        "Lead Source",
    ])

    ctry_col = _find_col(base, [
        "Country",
        "Deal Country",
        "Country Name",
    ])

    cal_slot_col = _find_col(base, [
        "Calibration Slot (Deal)",
        "Calibration slot (Deal)",
        "Calibration Slot",
    ])

    deal_name_col = _find_col(base, [
        "Deal Name",
        "Deal name",
        "Deal_Name",
        "Deal Title",
    ...
    # --- Date helpers and core fields ---
    def _to_dt(s):
        return pd.to_datetime(s, errors="coerce", dayfirst=True)

    base["_cd"] = _to_dt(base[create_col]) if create_col else pd.NaT
    base["_payd"] = _to_dt(base[pay_col]) if pay_col else pd.NaT
    base["_fcd"] = _to_dt(base[first_cal_col]) if first_cal_col else pd.NaT
    base["_rcd"] = _to_dt(base[cal_resch_col]) if cal_resch_col else pd.NaT

    if cal_book_col:
        base["_bkd"] = _to_dt(base[cal_book_col])
    else:
        base["_bkd"] = pd.NaT

    if done_col:
        base["_done"] = _to_dt(base[done_col])
    else:
        base["_done"] = pd.NaT
    ...
    metric_options = [
        "Deal count",
        "Enrollment",
        "Calibration booking count",
    ]

    add_cols_options = [
        "First calibration date count",
        "Calibration reschedule count",
        "Calibration done count",
    ]
    ...
    def _month_bounds(d):
        ...
        return (
            ...
            pd.Timestamp(d.year, d.month, monthrange(d.year, d.month)[1])
        )

    # ---------- Global view mode ----------
    view_mode = st.radio(
        "View mode",
        ["A1/B1/C1 basis", "JetLearn Deal Source & Country basis"],
        index=0,
        horizontal=True,
        help="Choose whether to filter globally by A1/B1/C1 segment or by JetLearn Deal Source & Country groups."
    )

    filtered = base.copy()

    # ----- JetLearn Deal Source & Country basis with multiple groups -----
    if view_mode == "JetLearn Deal Source & Country basis" and src_col and ctry_col:
        if "sr_pair_rows" not in st.session_state:
            st.session_state["sr_pair_rows"] = 1

        n_rows = max(1, int(st.session_state.get("sr_pair_rows", 1)))
        all_src_vals = sorted({str(x) for x in base[src_col].dropna().unique()})
        all_ctry_vals = sorted({str(x) for x in base[ctry_col].dropna().unique()})

        st.caption("Define one or more (JetLearn Deal Source, Countries) groups. Data used is the UNION of all groups.")

        group_masks = []

        for i in range(n_rows):
            col_src_sel, col_ctry_sel = st.columns([1.4, 2.0])

            with col_src_sel:
                src_sel = st.selectbox(
                    f"JetLearn Deal Source {i+1}",
                    ["All"] + all_src_vals,
                    key=f"sr_src_{i}"
                )

            # Country options depend on chosen source
            if src_sel == "All":
                ctry_options = all_ctry_vals
            else:
                sub_src = base[base[src_col].astype(str) == src_sel]
                ctry_options = sorted({str(x) for x in sub_src[ctry_col].dropna().unique()}) or all_ctry_vals

            with col_ctry_sel:
                ctry_opts = ["All"] + ctry_options
                sel_ctry = st.multiselect(
                    f"Countries for {src_sel if src_sel != 'All' else 'All sources'}",
                    ctry_opts,
                    default=["All"],
                    key=f"sr_ctry_{i}"
                )
            ...
            if src_sel == "All":
                if "All" in sel_ctry:
                    ...
                    m = base[ctry_col].astype(str).isin(sel_ctry)
            else:
                src_m = base[src_col].astype(str) == src_sel
                if "All" in sel_ctry:
                    m = src_m
                else:
                    m = src_m & base[ctry_col].astype(str).isin(sel_ctry)

            group_masks.append(m)

        col_add, col_reset = st.columns([1, 1])
        with col_add:
            if st.button("+ Add Source/Country group", key="sr_add_pair") and n_rows < 10:
                st.session_state["sr_pair_rows"] = n_rows + 1
        with col_reset:
            if st.button("Reset groups", key="sr_reset_pair"):
                for j in range(n_rows + 5):
                    st.session_state.pop(f"sr_src_{j}", None)
                    st.session_state.pop(f"sr_ctry_{j}", None)
                st.session_state["sr_pair_rows"] = 1
                group_masks = []

        if group_masks:
            mask_union = group_masks[0]
            for m in group_masks[1:]:
                mask_union = mask_union | m
            filtered = base[mask_union]
        else:
            filtered = base.copy()

    elif view_mode == "JetLearn Deal Source & Country basis":
        st.warning("JetLearn Deal Source or Country column not found. Showing all data.")
        filtered = base.copy()

    # ----- A1/B1/C1 basis -----
    if view_mode == "A1/B1/C1 basis":
        seg_col = _find_col(filtered, [
            "A1/B1/C1",
            "A1_B1_C1",
            "Segment (A1/B1/C1)",
            "A1 B1 C1",
        ])
        seg_opts = ["All", "A1", "B1", "C1"]
        seg_choice = st.selectbox(
            "A1/B1/C1 segment",
            seg_opts,
            index=0,
            help="Filter using A1/B1/C1 band if available."
        )
        if seg_choice != "All":
            if seg_col:
                filtered = filtered[filtered[seg_col].astype(str) == seg_choice]
            else:
                st.warning("A1/B1/C1 column not found; showing all data (no segment filter).")

    # ----- Optional: only rows with Calibration Slot (Deal) present -----
    if cal_slot_col:
        only_with_slot = st.checkbox(
            "Show only rows with Calibration Slot (Deal) present",
            value=False,
            help="When enabled, both Window A and Window B include only deals where Calibration Slot (Deal) has a non-blank value."
        )
        if only_with_slot:
            filtered = filtered[filtered[cal_slot_col].astype(str).str.strip() != ""]

    # ----- Optional: exclude specific Deal Names (All minus selected) -----
    if deal_name_col:
        enable_exclude_dn = st.checkbox(
            "Exclude specific Deal Names",
            value=False,
            help="When enabled, select Deal Names to remove from all calculations (Windows A & B and comparison)."
        )
        if enable_exclude_dn:
            dn_vals = sorted({str(x) for x in filtered[deal_name_col].dropna().unique()})
            to_exclude = st.multiselect(
                "Deal Names to exclude",
                dn_vals,
                key="sr_exclude_dealnames"
            )
            if to_exclude:
                filtered = filtered[~filtered[deal_name_col].astype(str).isin(to_exclude)]

    if filtered.empty:
        st.info("No data after applying global filters.")
        return

    # ---------- Per-window renderer ----------
    def _run_window(label: str, key_prefix: str, work_base: pd.DataFrame):
        st.markdown(f"### Window {label}")

        work = work_base.copy()

        # --- Controls row 1: Threshold & Mode ---
        c1, c2 = st.columns([2, 1.3])

        with c1:
            if thr_col:
                thr_vals = sorted({str(x) for x in work[thr_col].dropna().unique()})
                sel_thr = st.multiselect(
                    "New lead threshold values",
                    thr_vals,
                    default=thr_vals,
                    help="Filter by New Deal/Lead threshold buckets.",
                    key=f"{key_prefix}_thr"
                )
            else:
                sel_thr = None
                st.caption("No 'New Lead/Threshold' column detected — all thresholds included.")
        ...
        with c2:
            mode = st.radio(
                ...
                help="MTD: filter window on Create Date. Cohort: window on each event date."
            )

        # --- Controls row 2: Group, Metrics, Additional Columns ---
        c4, c5, c6 = st.columns([1.7, 1.7, 1.6])

        with c4:
            group_choice = st.selectbox(
                "Group by",
                ["Academic Counsellor", "JetLearn Deal Source", "Country", "(Overall)"],
                index=0,
        ...
        with c5:
            sel_metrics = st.multiselect(
                "Metrics",
                metric_options,
                default=["Deal count", "Enrollment"],
                key=f"{key_prefix}_metrics"
            )

        with c6:
            sel_add_cols = st.multiselect(
                "Additional columns",
                add_cols_options,
                default=[],
                key=f"{key_prefix}_addcols",
                help="Include extra calibration-related counts."
            )
        ...
        if not sel_metrics and not sel_add_cols:
            st.warning("Select at least one metric or additional column.")
            return None

        # --- Date range presets ---
        presets = ["This Month", "Last Month", "Last 90 Days", "Today", "Yesterday", "Custom"]
        preset = st.radio(
            "Date range",
            presets,
            index=0,
        ...
        if preset == "Today":
            ...
            start, end = today - pd.Timedelta(days=1), today - pd.Timedelta(days=1)
        elif preset == "This Month":
            start, end = _month_bounds(today)
        elif preset == "Last Month":
            first_this = pd.Timestamp(today.year, today.month, 1)
            last_prev = first_this - pd.Timedelta(days=1)
            start, end = _month_bounds(last_prev)
        elif preset == "Last 90 Days":
            start, end = today - pd.Timedelta(days=89), today
        else:
            c7, c8 = st.columns(2)
            with c7:
                start = st.date_input(
                    "Start date",
                    value=today - pd.Timedelta(days=29),
        ...
        # --- Threshold filter ---
        if thr_col and sel_thr is not None:
            work = work[work[thr_col].astype(str).isin(sel_thr)]

        if work.empty:
            st.info("No data after filters for this window.")
            return None

        # --- Grouping ---
        if group_choice == "(Overall)":
            work["_grp"] = "(Overall)"
        elif group_choice == "Academic Counsellor" and ac_col:
            work["_grp"] = work[ac_col].fillna("Unknown")
        elif group_choice == "JetLearn Deal Source" and src_col:
            work["_grp"] = work[src_col].fillna("Unknown")
        elif group_choice == "Country" and ctry_col:
            work["_grp"] = work[ctry_col].fillna("Unknown")
        else:
            work["_grp"] = "(Overall)"
        ...
        for g, sub in work.groupby("_grp"):
            rec = {"Group": g}

            # Base MTD on Create Date
            if mode == "MTD" and create_col:
                base_mtd = (sub["_cd"] >= start) & (sub["_cd"] <= end)
            else:
                base_mtd = pd.Series(True, index=sub.index)

            # Deal count
            if "Deal count" in sel_metrics:
                if mode == "MTD":
                    rec["Deal count"] = int(base_mtd.sum())
                else:
                    rec["Deal count"] = int(((sub["_cd"] >= start) & (sub["_cd"] <= end)).sum())

            # Enrollment
            if "Enrollment" in sel_metrics:
                if mode == "MTD":
                    rec["Enrollment"] = int((base_mtd & sub["_payd"].notna()).sum())
                else:
                    rec["Enrollment"] = int(((sub["_payd"] >= start) & (sub["_payd"] <= end)).sum())

            # Calibration booking count
            if "Calibration booking count" in sel_metrics:
                if mode == "MTD":
                    rec["Calibration booking count"] = int((base_mtd & sub["_bkd"].notna()).sum())
                else:
                    rec["Calibration booking count"] = int(((sub["_bkd"] >= start) & (sub["_bkd"] <= end)).sum())

            # First calibration date count
            if ("First calibration date count" in sel_metrics) or ("First calibration date count" in sel_add_cols):
                if mode == "MTD":
                    rec["First calibration date count"] = int((base_mtd & sub["_fcd"].notna()).sum())
                else:
                    rec["First calibration date count"] = int(((sub["_fcd"] >= start) & (sub["_fcd"] <= end)).sum())

            # Calibration reschedule count
            if ("Calibration reschedule count" in sel_metrics) or ("Calibration reschedule count" in sel_add_cols):
                if mode == "MTD":
                    rec["Calibration reschedule count"] = int((base_mtd & sub["_rcd"].notna()).sum())
                else:
                    rec["Calibration reschedule count"] = int(((sub["_rcd"] >= start) & (sub["_rcd"] <= end)).sum())

            # Calibration done count (Calibration Done Date)
            if "Calibration done count" in sel_add_cols:
                if mode == "MTD":
                    rec["Calibration done count"] = int((base_mtd & sub["_done"].notna()).sum())
                else:
                    rec["Calibration done count"] = int(((sub["_done"] >= start) & (sub["_done"] <= end)).sum())

            # Conversion %
            if ("Deal count" in rec and "Enrollment" in rec and rec.get("Deal count", 0) > 0):
                rec["Enrollment / Deal %"] = round(
                    rec["Enrollment"] * 100.0 / rec["Deal count"],
                    2
                )
        ...
        if not rows:
            st.info("No aggregations computed — adjust filters.")
            return None
        ...
        for m in metric_options:
            if m in sel_metrics and m in agg.columns and m not in keep_cols:
                keep_cols.append(m)

        for m in add_cols_options:
            if m in sel_add_cols and m in agg.columns and m not in keep_cols:
                keep_cols.append(m)

        if "Enrollment / Deal %" in agg.columns and "Enrollment / Deal %" not in keep_cols:
            keep_cols.append("Enrollment / Deal %")

        agg = agg[keep_cols]

        # --- Table ---
        st.markdown(f"#### Window {label} table")
        st.dataframe(
            agg.sort_values("Group"),
            use_container_width=True,
            hide_index=True
        )

        st.download_button(
            f"Download CSV — Window {label}",
            agg.to_csv(index=False).encode("utf-8"),
            f"super_report_window_{label.lower()}.csv",
            "text/csv",
            key=f"{key_prefix}_download"
        )

        # --- Chart ---
        value_cols = [c for c in agg.columns if c != "Group"]
        if not value_cols:
            return agg
        ...
        if chart_type == "Bar":
            chart = alt.Chart(plot_data).mark_bar().encode(
                ...
                tooltip=["Group:N", "Metric:N", "Value:Q"]
            )
            ...
            chart = alt.Chart(plot_data).mark_area(opacity=0.6).encode(
                x=alt.X("Group:N"),
                y=alt.Y("Value:Q"),
                color=alt.Color("Metric:N"),
                tooltip=["Group:N", "Metric:N", "Value:Q"]
        ...
        st.markdown(f"#### Window {label} chart")
        st.altair_chart(chart.properties(height=420), use_container_width=True)

        return agg

    # ---------- Two side-by-side windows for comparison ----------
    col_a, col_b = st.columns(2)
    with col_a:
        agg_a = _run_window("A", "A", filtered)
    with col_b:
        agg_b = _run_window("B", "B", filtered)

    # Combined comparison table
    if agg_a is not None and agg_b is not None:
        try:
            comp = agg_a.merge(agg_b, on="Group", how="outer", suffixes=("_A", "_B"))
            st.markdown("### Comparison table — Window A vs Window B")
            st.dataframe(
                comp.sort_values("Group"),
                use_container_width=True,
                hide_index=True
            )
            st.download_button(
                ...
                "super_report_comparison_A_vs_B.csv",
                "text/csv",
                key="super_report_comp_download"
            )
        except Exception as _e:
            st.caption(f"Comparison view not available: {_e}")
...
try:
    if master == "Marketing" and st.session_state.get('nav_sub') == "Super report":
        _df_any = None
        for _nm in ("dff","df_f","df"):
            _cand = globals().get(_nm, None)
            if _cand is not None:
                ...
                break
        if _df_any is not None:
            _render_marketing_super_report(_df_any)
        else:
            st.error("No dataframe available to render Super report.")
except Exception as _e:
    import traceback, streamlit as st
    st.error(f"Super report failed: {_e}")
...
# ================================
# Marketing ▶ Intern vs AC
# ================================
def _render_marketing_intern_vs_ac(df):
    """
    Intern vs AC comparison (event-date based).

    Counts use ONLY event dates:
      - First Calibration Scheduled
      - Calibration Rescheduled
      - Calibration Done
      - [Trigger] - Calibration Booking Date
      - Enrollments (Payment Received Date)

    Two windows (A & B):
      - Today / Yesterday / This Month / Last Month / Custom

    Filters:
      - Segment type: Intern / AC (multi-select)
      - Segment names:
          * Intern: multi-select or All
          * AC: multi-select or All
      - Breakdown by (drives the output table):
          * Overall
          * Country
          * JetLearn Deal Source
          * Intern Name
    ...
    The output table always matches the Breakdown selection:
      - Overall          → aggregate by segment & window
      - Country          → per country
      - JetLearn Source  → per source
      - Intern Name      → per intern
      - AC Name          → per AC
    """
    import streamlit as st
    import pandas as pd
    ...
    # helpers
    def _find_col(_df, candidates):
        # exact
        for c in candidates:
            if c in _df.columns:
                return c
        # case-insensitive
        low = {c.lower(): c for c in _df.columns}
        for c in candidates:
            if c.lower() in low:
                return low[c.lower()]
        # fuzzy contains
    ...
    # key columns
    intern_col = _find_col(df, [
        "Sales - Intern", "Sales Intern", "Sales intern",
        "Sales_Intern", "Sales Intern Name", "Intern Name"
    ])
    ac_col = _find_col(df, [
        "Student/Academic Counsellor", "Student / Academic Counsellor",
        "Academic Counsellor", "AC Name", "Owner"
    ])

    first_cal_col = _find_col(df, [
        "First Calibration Scheduled Date", "First Trial Scheduled Date",
        "First Trial Date"
    ])
    resched_col = _find_col(df, [
        "Calibration Rescheduled Date", "Trial Rescheduled Date",
        "Rescheduled Date"
    ])
    done_col = _find_col(df, [
        "Calibration Done Date", "Trial Done Date",
        "Trial Completed Date"
    ])
    trig_col = _find_col(df, [
        "[Trigger] - Calibration Booking Date",
        "Trigger - Calibration Booking Date",
        "[Trigger] - Trial Booking Date"
    ])
    pay_col = _find_col(df, [
        "Payment Received Date", "Payment Received date", "Deal Won Date",
        "Enrolment Date", "Enrolled On", "Payment_Received_Date"
    ])

    country_col = _find_col(df, ["Country", "Country Name", "Geo", "Region"])
    src_col = _find_col(df, [
        "JetLearn Deal Source", "Jetlearn Deal Source", "Deal Source",
        "Original Source", "Original Source 1", "Lead Source"
    ])

    if not intern_col and not ac_col:
        st.error("Neither 'Sales - Intern' nor 'Academic Counsellor' columns were found.")
        return

    # strict name check
    def _has_name(series):
        if series is None:
            return pd.Series(False, index=df.index)
        s = series.astype(str).str.strip()
        invalid = {"", "nan", "na", "none", "null", "nil"}
        return (~series.isna()) & (~s.str.lower().isin(invalid))

    # segment labels
    seg = pd.Series("Other", index=df.index)
    if intern_col:
        is_intern = _has_name(df[intern_col])
        seg[is_intern] = "Intern"
    else:
        is_intern = pd.Series(False, index=df.index)

    if ac_col:
        is_ac = _has_name(df[ac_col]) & ~is_intern
        seg[is_ac] = "AC"
    else:
        is_ac = pd.Series(False, index=df.index)

    mask_seg = seg.isin(["Intern", "AC"])
    if not mask_seg.any():
        st.warning("No rows tagged as Sales Intern or Academic Counsellor with valid names.")
        return
    ...
    # event dates
    first_cal_dt = _coerce_dt(df[first_cal_col]) if first_cal_col else pd.Series(pd.NaT, index=df.index)
    resched_dt = _coerce_dt(df[resched_col]) if resched_col else pd.Series(pd.NaT, index=df.index)
    done_dt = _coerce_dt(df[done_col]) if done_col else pd.Series(pd.NaT, index=df.index)
    trig_dt = _coerce_dt(df[trig_col]) if trig_col else pd.Series(pd.NaT, index=df.index)
    pay_dt = _coerce_dt(df[pay_col]) if pay_col else pd.Series(pd.NaT, index=df.index)

    st.subheader("Intern vs AC — Event-date based comparison")
    ...
    # 2) Segment name filters (per type)
    selected_intern_names = None
    selected_ac_names = None

    col_intern, col_ac = st.columns(2)
    if "Intern" in selected_segs and intern_col:
        intern_names = sorted(
            df.loc[seg == "Intern", intern_col].dropna().astype(str).str.strip().unique()
        )
        with col_intern:
            intern_choices = ["All"] + intern_names
            selected_intern_names = st.multiselect(
                "Select Intern(s)",
                intern_choices,
                default=["All"],
                key="iva_intern_names"
            )

    if "AC" in selected_segs and ac_col:
        ac_names = sorted(
            df.loc[seg == "AC", ac_col].dropna().astype(str).str.strip().unique()
        )
        with col_ac:
            ac_choices = ["All"] + ac_names
            selected_ac_names = st.multiselect(
                "Select Academic Counsellor(s)",
                ac_choices,
                default=["All"],
                key="iva_ac_names"
            )
    ...
    breakdown = st.radio(
        "Breakdown by",
        ["Overall", "Country", "JetLearn Deal Source", "Intern Name", "AC Name"],
        index=0,
        horizontal=True,
        key="iva_breakdown"
    )
    ...
    dim_col = None
    if breakdown == "Country" and country_col:
        dim_col = country_col
    elif breakdown == "JetLearn Deal Source" and src_col:
        dim_col = src_col
    elif breakdown == "Intern Name" and intern_col:
        dim_col = intern_col
    elif breakdown == "AC Name" and ac_col:
        dim_col = ac_col

    # multi-select values for breakdown (except Overall)
    dim_values = None
    if dim_col is not None:
        all_vals = sorted(df[dim_col].dropna().astype(str).unique())
        choices = ["All"] + all_vals
        dim_values = st.multiselect(
            f"Select {breakdown} value(s)",
            choices,
            default=["All"],
            key="iva_dim_values"
        )
        if (not dim_values) or ("All" in dim_values):
            dim_values = None  # treat as use all

    # 4) Window helpers (event-date only)
    def _month_bounds(d: date):
        from calendar import monthrange
        return date(d.year, d.month, 1), date(d.year, d.month, monthrange(d.year, d.month)[1])
    ...
    def _select_window(label: str, key_prefix: str):
        today = date.today()
        c1, c2 = st.columns(2)
        with c1:
            preset = st.radio(
        ...
        if preset == "Today":
            ...
            if start_d > end_d:
                st.error(f"{label}: Start date cannot be after end date.")
        start_ts = pd.to_datetime(start_d)
        end_ts = pd.to_datetime(end_d) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
        return start_ts, end_ts

    st.markdown("**Configure comparison windows (event-date based)**")
    wA_start, wA_end = _select_window("Window A", "iva_A")
    wB_start, wB_end = _select_window("Window B", "iva_B")

    # 5) Metrics with all filters (shape according to Breakdown by)
    def _metrics_for(seg_name: str, dim_val, start, end):
        if seg_name not in selected_segs:
            return None

        base_mask = (seg == seg_name)

        # segment-name filters
        if seg_name == "Intern" and intern_col and selected_intern_names:
            if "All" not in selected_intern_names:
                base_mask &= df[intern_col].astype(str).str.strip().isin(selected_intern_names)
        if seg_name == "AC" and ac_col and selected_ac_names:
            if "All" not in selected_ac_names:
                base_mask &= df[ac_col].astype(str).str.strip().isin(selected_ac_names)

        # breakdown-specific value
        if dim_col is not None and dim_val is not None:
            base_mask &= df[dim_col].astype(str).str.strip() == dim_val
        ...
        if not base_mask.any():
            return {
                ...
                "Trig Booking": 0,
                "Enrollments": 0,
                "% CalDone / FirstCal": 0.0,
                "% Enrol / CalDone": 0.0,
            }

        fc_mask = base_mask & first_cal_dt.between(start, end) if first_cal_col else pd.Series(False, index=df.index)
        rs_mask = base_mask & resched_dt.between(start, end) if resched_col else pd.Series(False, index=df.index)
        dn_mask = base_mask & done_dt.between(start, end) if done_col else pd.Series(False, index=df.index)
        tg_mask = base_mask & trig_dt.between(start, end) if trig_col else pd.Series(False, index=df.index)
        py_mask = base_mask & pay_dt.between(start, end) if pay_col else pd.Series(False, index=df.index)

        n_fc = int(fc_mask.sum())
        n_rs = int(rs_mask.sum())
        n_dn = int(dn_mask.sum())
        n_tg = int(tg_mask.sum())
        n_py = int(py_mask.sum())

        pct_dn_fc = float(round((n_dn / n_fc) * 100, 2)) if n_fc > 0 else 0.0
        pct_py_dn = float(round((n_py / n_dn) * 100, 2)) if n_dn > 0 else 0.0
        ...
        return {
            ...
            "Trig Booking": n_tg,
            "Enrollments": n_py,
            "% CalDone / FirstCal": pct_dn_fc,
            "% Enrol / CalDone": pct_py_dn,
        }
    ...
    if dim_col is None:
        # Overall
        for label, (ws, we) in {
            "Window A": (wA_start, wA_end),
            "Window B": (wB_start, wB_end),
        }.items():
            for seg_name in selected_segs:
                met = _metrics_for(seg_name, None, ws, we)
                if met is None:
                    continue
                rows.append({
                    "Window": label,
                    ...
                    "Breakdown Value": "Overall",
                    "First Cal": met["First Cal"],
                    "Cal Resched": met["Cal Resched"],
                    "Cal Done": met["Cal Done"],
                    "Trig Booking": met["Trig Booking"],
                    "Enrollments": met["Enrollments"],
                    "% CalDone / FirstCal": met["% CalDone / FirstCal"],
                    "% Enrol / CalDone": met["% Enrol / CalDone"],
                })
    else:
        # Country / Source / Intern Name / AC Name
        if dim_values:
            dim_list = dim_values
        else:
            dim_list = sorted(df[dim_col].dropna().astype(str).unique())

        if breakdown == "Country":
            btype = "Country"
        elif breakdown == "JetLearn Deal Source":
            btype = "JetLearn Deal Source"
        elif breakdown == "Intern Name":
            btype = "Intern Name"
        elif breakdown == "AC Name":
            btype = "AC Name"
        else:
            btype = breakdown
        ...
        for label, (ws, we) in {
            ...
            for seg_name in selected_segs:
                for dim_val in dim_list:
                    met = _metrics_for(seg_name, dim_val, ws, we)
                    if met is None:
                        continue
                    rows.append({
                        "Window": label,
    ...
    if not summary_df.empty:
        try:
            ...
            chart = alt.Chart(summary_df).mark_bar().encode(
                ...
                y=alt.Y("First Cal:Q", title="First Cal count"),
                color=alt.Color("Window:N"),
                column=alt.Column("Window:N", title="Window")
            )
            st.altair_chart(chart, use_container_width=True)
        except Exception:
            pass
    ...
    st.download_button(
        "Download CSV — Intern vs AC (event-date based, filtered)",
        data=summary_df.to_csv(index=False).encode("utf-8"),
        file_name="intern_vs_ac_event_date_based_filtered.csv",
        mime="text/csv",
        key="iva_dl"
    )

# ---- Dispatcher for Marketing ▸ Intern vs AC
if st.session_state.get("nav_master") == "Marketing" and st.session_state.get("nav_sub") == "Intern vs AC":
    _df_any = None
    for _nm in ("df_f", "dff", "df"):
        _cand = globals().get(_nm, None)
        if _cand is not None:
            _df_any = _cand
            break
    if _df_any is not None:
        _render_marketing_intern_vs_ac(_df_any)
    else:
        st.error("No dataframe available to render Intern vs AC.")

# ================================
# Marketing ▶ Plan vs Actual
# ================================
def _render_marketing_plan_vs_actual(
    df_f,
    create_col: str | None = None,
    pay_col: str | None = None,
    first_cal_col: str | None = None,
    cal_resched_col: str | None = None,
    cal_done_col: str | None = None,
):
    import streamlit as st
    ...
    st.subheader("Marketing — Plan vs Actual")
    ...
    # ---------- helpers ----------
    def _col_exists(df_in, candidates):
        for c in candidates:
            if c in df_in.columns:
                return c
        lower = {c.lower(): c for c in df_in.columns}
        for c in candidates:
            lc = c.lower()
            if lc in lower:
                return lower[lc]
    ...
    def _parse_dates(col_name):
        if not col_name or col_name not in df.columns:
            return pd.Series(pd.NaT, index=df.index)
        if "_dt_parse_dayfirst" in globals():
            return globals()["_dt_parse_dayfirst"](df[col_name])
        return pd.to_datetime(df[col_name], errors="coerce", dayfirst=True)

    # ---------- core columns ----------
    deal_source_col = _col_exists(df, [
        "JetLearn Deal Source", "JetLearn deal source", "deal_source", "Jetlearn Deal Source"
    ])
    country_col = _col_exists(df, [
        "Country", "Student Country", "Contact: Country/Region", "country"
    ])

    if not deal_source_col:
        st.error("JetLearn Deal Source column not found.")
        return

    if not create_col:
        create_col = _col_exists(df, ["Create Date", "Created Date", "Create date"])
    if not pay_col:
        pay_col = _col_exists(df, ["Payment Received Date", "Payment date", "Payment Date"])

    _create = _parse_dates(create_col)
    _pay = _parse_dates(pay_col)

    # Calibration-related columns (optional)
    if not first_cal_col:
        first_cal_col = _col_exists(df, [
            "First Calibration Scheduled Date",
            "First Calibration Schedule Date",
            "First Calibration Date",
        ])
    if not cal_resched_col:
        cal_resched_col = _col_exists(df, [
            "Calibration Rescheduled Date",
            "Cal Rescheduled Date",
        ])
    if not cal_done_col:
        cal_done_col = _col_exists(df, [
            "Calibration Done Date",
            "Cal Done Date",
        ])

    _fc = _parse_dates(first_cal_col) if first_cal_col else pd.Series(pd.NaT, index=df.index)
    _cr = _parse_dates(cal_resched_col) if cal_resched_col else pd.Series(pd.NaT, index=df.index)
    _cd = _parse_dates(cal_done_col) if cal_done_col else pd.Series(pd.NaT, index=df.index)

    # ---------- optional extra metrics & filter ----------
    metric_defs = {}
    if not _fc.isna().all():
        metric_defs["First Calibration Count"] = (
            "Planned First Cal Count",
            "Actual First Cal Count",
            "Gap First Cal Count",
            _fc,
        )
    if not _cr.isna().all():
        metric_defs["Calib Reschedule Count"] = (
            "Planned Calib Resch Count",
            "Actual Calib Resch Count",
            "Gap Calib Resch Count",
            _cr,
        )
    if not _cd.isna().all():
        metric_defs["Calib Done Count"] = (
            "Planned Calib Done Count",
            "Actual Calib Done Count",
            "Gap Calib Done Count",
            _cd,
        )

    extra_selected = st.multiselect(
        "Optional indicators (besides Deals & Enrolments)",
        options=list(metric_defs.keys()),
        default=list(metric_defs.keys()),
        key="pva_extra_metrics",
    )

    # User sees MTD / Cohort. Flip internal logic per your preference:
    mode_selection = st.radio(
        "Mode",
        ["MTD", "Cohort"],
        index=0,
        key="pva_mode",
        help="Choose how to compute Actuals (display flips internally as requested).",
    )
    mode = "Cohort" if mode_selection == "MTD" else "MTD"

    today = date.today()
    month_start = pd.Timestamp(today.replace(day=1)).normalize()
    month_end = pd.Timestamp(today).normalize()

    # ---------- 1️⃣ Plan (editable; non-persistent) ----------
    st.markdown("##### 1️⃣ Plan (editable; non-persistent)")

    lookback_options = [1, 2, 3, 6, 12, 18, 24]
    lb = st.selectbox(
        "Lookback period (months) for historical conversion %",
        options=lookback_options,
        index=lookback_options.index(3) if 3 in lookback_options else 0,
        key="pva_lb_months",
    )

    rows_planned = st.number_input(
        "Number of planning rows",
        min_value=1,
        max_value=40,
        value=5,
        step=1,
    ...
    if relativedelta:
        ...
        hist_start = (hist_end - relativedelta(months=lb)).normalize()
    else:
        hist_end = pd.Timestamp(today).normalize()
        hist_start = (hist_end - pd.Timedelta(days=30 * lb)).normalize()

    all_sources = sorted(df[deal_source_col].dropna().astype(str).unique())
    all_countries = sorted(df[country_col].dropna().astype(str).unique()) if country_col else []

    plan_records = []

    for i in range(int(rows_planned)):
        c1, c2, c3 = st.columns([2, 3, 2])
        with c1:
            src = st.selectbox(
                f"Row {i+1}: JetLearn Deal Source",
                options=[""] + all_sources,
                key=f"pva_src_{i}",
            )
        with c2:
            countries = st.multiselect(
                f"Row {i+1}: Country (type to search; blank = ALL)",
                options=all_countries,
                key=f"pva_ctry_{i}",
            )
        with c3:
            deals_plan = st.number_input(
                f"Row {i+1}: Planned Deals",
                min_value=0.0,
                value=0.0,
                step=1.0,
                key=f"pva_plan_deals_{i}",
            )

        if not src or deals_plan <= 0:
            continue

        mask_hist = df[deal_source_col].astype(str).eq(src)
        if countries and country_col:
            mask_hist &= df[country_col].astype(str).isin(countries)

        mask_deals_hist = mask_hist & _create.between(hist_start, hist_end, inclusive="both")
        hist_idx = df.index[mask_deals_hist]
        deals_hist = len(hist_idx)

        if deals_hist > 0:
            enroll_hist = int(_pay.loc[hist_idx].notna().sum())
            conv_hist = enroll_hist / deals_hist if deals_hist > 0 else 0.0
        else:
            enroll_hist = 0
            conv_hist = 0.0

        planned_enrol = deals_plan * conv_hist

        # Planned extra metrics based on lookback ratios
        extra_plan_vals = {}
        for label in extra_selected:
            plan_key, _, _, series = metric_defs[label]
            try:
                hist_events = int(
                    series.loc[
                        mask_hist & series.between(hist_start, hist_end, inclusive="both")
                    ].notna().sum()
                )
            except Exception:
                hist_events = 0
            ratio = (hist_events / deals_hist) if deals_hist > 0 else 0.0
            extra_plan_vals[plan_key] = round(deals_plan * ratio, 2)

        rec = {
            "Row": i + 1,
            "JetLearn Deal Source": src,
            "Countries (Planned)": ", ".join(countries) if countries else "ALL",
            "Lookback (months)": lb,
            "Hist. Deals": deals_hist,
            "Hist. Enrolments": enroll_hist,
            "Hist. Conv%": round(conv_hist * 100, 2),
            "Planned Deals": deals_plan,
            "Planned Enrolments": round(planned_enrol, 2),
        }
        rec.update(extra_plan_vals)
    ...
    if not plan_df.empty:
        ...
        st.info("Add at least one valid row to see planned numbers.")

    # ---------- 2️⃣ Actual Snapshot (prefilled by Deal Source) ----------
    st.markdown("##### 2️⃣ Actual Snapshot (prefilled)")

    try:
        snap = df.copy()
        if mode == "MTD":
            deal_mask_snap = _create.between(month_start, month_end, inclusive="both")
            enrol_mask_snap = _pay.between(month_start, month_end, inclusive="both")
        else:
            cohort_mask_snap = _create.between(month_start, month_end, inclusive="both")
            deal_mask_snap = cohort_mask_snap
            enrol_mask_snap = cohort_mask_snap & _pay.notna()

        snap["is_deal"] = deal_mask_snap
        snap["is_enrol"] = enrol_mask_snap

        extra_cols = {}
        for label in extra_selected:
            _, actual_key, _, series = metric_defs[label]
            if series is None:
                continue
            if mode == "MTD":
                m = series.between(month_start, month_end, inclusive="both")
            else:
                m = cohort_mask_snap & series.notna()
            col_name = f"snap_{actual_key}"
            snap[col_name] = m
            extra_cols[col_name] = actual_key

        activity_mask = snap["is_deal"] | snap["is_enrol"]
        for col_name in extra_cols.keys():
            activity_mask = activity_mask | snap[col_name]

        agg_cols = {"is_deal": "Deals", "is_enrol": "Enrolments"}
        agg_cols.update({k: v for k, v in extra_cols.items()})

        grp = (
            snap.groupby(deal_source_col, dropna=False)[list(agg_cols.keys())]
            .sum()
            .reset_index()
        ).rename(columns={deal_source_col: "JetLearn Deal Source", **agg_cols})

        if country_col:
            mapping = {}
            for src in grp["JetLearn Deal Source"]:
                m_src = (df[deal_source_col].astype(str) == str(src)) & activity_mask
                ctrs = df.loc[m_src, country_col].dropna().astype(str).unique()
                mapping[src] = ", ".join(sorted(ctrs)) if len(ctrs) else "NA"
            grp["Countries (Actual Snapshot)"] = grp["JetLearn Deal Source"].map(mapping)

        if "Deals" in grp.columns and "Enrolments" in grp.columns:
            grp["Conv%"] = ((grp["Enrolments"] / grp["Deals"].where(grp["Deals"] > 0)).fillna(0) * 100)

        grp = grp.loc[(grp.get("Deals", 0) > 0) | (grp.get("Enrolments", 0) > 0)]
        if not grp.empty:
            st.dataframe(grp.sort_values("JetLearn Deal Source"), use_container_width=True, hide_index=True)
        else:
            st.info("No actual activity yet for current mode to prefill snapshot.")
    except Exception:
        pass

    # ---------- 2️⃣ Actuals vs Plan Rows ----------
    st.markdown("##### 2️⃣ Actuals vs Plan Rows")

    actual_records = []
    if not plan_df.empty:
        for _, r in plan_df.iterrows():
            src = r["JetLearn Deal Source"]
            planned_c = str(r.get("Countries (Planned)", "ALL"))
            sel_countries = [] if planned_c == "ALL" else [c.strip() for c in planned_c.split(",") if c.strip()]

            base_mask = df[deal_source_col].astype(str).eq(src)
            if sel_countries and country_col:
                base_mask &= df[country_col].astype(str).isin(sel_countries)

            if mode == "MTD":
                deals_mask = base_mask & _create.between(month_start, month_end, inclusive="both")
                enrol_mask = base_mask & _pay.between(month_start, month_end, inclusive="both")
            else:
                cohort_mask = base_mask & _create.between(month_start, month_end, inclusive="both")
                deals_mask = cohort_mask
                enrol_mask = cohort_mask & _pay.notna()

            deals_actual = int(deals_mask.sum())
            enrol_actual = int(enrol_mask.sum())
            conv_actual = (enrol_actual / deals_actual) * 100 if deals_actual > 0 else 0.0

            activity_mask = deals_mask | enrol_mask

            rec = {
                "Row": int(r["Row"]),
                "JetLearn Deal Source": src,
                "Countries (Planned)": planned_c,
                "Planned Deals": float(r.get("Planned Deals", 0)),
                "Planned Enrolments": float(r.get("Planned Enrolments", 0)),
                "MTD/Cohort Deals (Actual)": deals_actual,
                "MTD/Cohort Enrolments (Actual)": enrol_actual,
                "Hist. Conv%": float(r.get("Hist. Conv%", 0)),
                "MTD/Cohort Conv% (Actual)": round(conv_actual, 2),
            }

            for label in extra_selected:
                plan_key, actual_key, _, series = metric_defs[label]
                if series is not None:
                    if mode == "MTD":
                        m = base_mask & series.between(month_start, month_end, inclusive="both")
                    else:
                        m = deals_mask & series.notna()
                    rec[actual_key] = int(m.sum())
                    activity_mask = activity_mask | m

            if country_col:
                ctrs = df.loc[activity_mask, country_col].dropna().astype(str).unique()
            else:
                ctrs = []
            rec["Countries (Actual)"] = ", ".join(sorted(ctrs)) if len(ctrs) else "NA"
    ...
    elif not plan_df.empty:
        st.info("No matching actuals found for this mode / plan.")

    # ---------- 3️⃣ Gap & Comparison ----------
    st.markdown("##### 3️⃣ Gap & Comparison")

    if not plan_df.empty and not actual_df.empty:
        plan_gap = plan_df.copy()
        merged = pd.merge(
            plan_gap,
            actual_df,
            on=["Row", "JetLearn Deal Source"],
            how="left",
            suffixes=("_Plan", "_Actual"),
        )

        # --- Ensure required columns exist for charts (derive if missing) ---
        # Coerce numerics where present
        for _c in [
            "Planned Deals","Planned Enrolments",
            "MTD/Cohort Deals (Actual)","MTD/Cohort Enrolments (Actual)",
            "Hist. Conv%","MTD/Cohort Conv% (Actual)"
        ]:
            if _c in merged.columns:
                merged[_c] = pd.to_numeric(merged[_c], errors="coerce").fillna(0.0)

        # Derive Conversion % columns if missing
        if "Hist. Conv%" not in merged.columns and            "Planned Enrolments" in merged.columns and "Planned Deals" in merged.columns:
            merged["Hist. Conv%"] = (merged["Planned Enrolments"] / merged["Planned Deals"].replace(0, pd.NA))                                        .fillna(0.0) * 100.0

        if "MTD/Cohort Conv% (Actual)" not in merged.columns and            "MTD/Cohort Enrolments (Actual)" in merged.columns and "MTD/Cohort Deals (Actual)" in merged.columns:
            merged["MTD/Cohort Conv% (Actual)"] = (merged["MTD/Cohort Enrolments (Actual)"] /                                                     merged["MTD/Cohort Deals (Actual)"].replace(0, pd.NA))                                                    .fillna(0.0) * 100.0
    
        # Core numeric fields
        for col in [
            "Planned Deals",
            "Planned Enrolments",
            "MTD/Cohort Deals (Actual)",
            "MTD/Cohort Enrolments (Actual)",
            "Hist. Conv%",
            "MTD/Cohort Conv% (Actual)",
        ]:
            if col in merged.columns:
                merged[col] = merged[col].fillna(0).astype(float)

        merged["Gap Deals"] = merged.get("MTD/Cohort Deals (Actual)", 0) - merged.get("Planned Deals", 0)
        merged["Gap Enrolments"] = merged.get("MTD/Cohort Enrolments (Actual)", 0) - merged.get("Planned Enrolments", 0)
        merged["Gap Conv (pp)"] = merged.get("MTD/Cohort Conv% (Actual)", 0) - merged.get("Hist. Conv%", 0)

        # Extra metric gaps
        for label in extra_selected:
            plan_key, actual_key, gap_key, _ = metric_defs[label]
            if plan_key in merged.columns or actual_key in merged.columns:
                merged[plan_key] = merged.get(plan_key, 0).fillna(0).astype(float)
                merged[actual_key] = merged.get(actual_key, 0).fillna(0).astype(float)
                merged[gap_key] = merged[actual_key] - merged[plan_key]
        ...
        for label in extra_selected:
            plan_key, actual_key, gap_key, _ = metric_defs[label]
            for c in (plan_key, actual_key, gap_key):
        ...
        # Visual comparison metric (Plan vs Actual) – includes Deals, Enrolments, Conversion %, and optional metrics
        try:
            metric_map = {}
            def _add_metric(label, plan_col, act_col):
                if plan_col in merged.columns and act_col in merged.columns:
                    metric_map[label] = (plan_col, act_col)

            _add_metric("Deals", "Planned Deals", "MTD/Cohort Deals (Actual)")
            _add_metric("Enrolments", "Planned Enrolments", "MTD/Cohort Enrolments (Actual)")
            _add_metric("Conversion %", "Hist. Conv%", "MTD/Cohort Conv% (Actual)")
            _add_metric("First Calibration Count", "Planned First Cal Count", "Actual First Cal Count")
            _add_metric("Calib Reschedule Count", "Planned Calib Resch Count", "Actual Calib Resch Count")
            _add_metric("Calib Done Count", "Planned Calib Done Count", "Actual Calib Done Count")

            if metric_map:
                metric_label = st.selectbox(
                    "Visual comparison metric (Plan vs Actual)",
                    options=list(metric_map.keys()),
                    index=0,
                    key="pva_gap_chart_metric",
                )
                plan_col, act_col = metric_map[metric_label]

                chart_type = st.selectbox(
                    "Chart type",
                    ["Bar", "Line", "Stacked Bar"],
                    index=0,
                    key="pva_gap_chart_type",
                )

                chart_df = merged[["JetLearn Deal Source", plan_col, act_col]].copy()
                chart_df = chart_df.melt(
                    "JetLearn Deal Source",
                    var_name="Type",
                    value_name="Value",
                )
                chart_df["Type"] = chart_df["Type"].replace({plan_col: "Plan", act_col: "Actual"})

                import altair as alt
                if chart_type == "Bar":
                    chart = alt.Chart(chart_df).mark_bar().encode(
                        x=alt.X("JetLearn Deal Source:N", sort="-y"),
                        y=alt.Y("Value:Q"),
        ...
        # Combined comparison: Deals, Enrolments, and Conversion %
        try:
            import altair as alt
            req_cols = [
                "Planned Deals", "MTD/Cohort Deals (Actual)",
                "Planned Enrolments", "MTD/Cohort Enrolments (Actual)",
                "Hist. Conv%", "MTD/Cohort Conv% (Actual)"
            ]
            if all(c in merged.columns for c in req_cols):
                ...
                bars_df = bars_df.rename(columns={
                    "Planned Deals": "Deals (Plan)",
                    "MTD/Cohort Deals (Actual)": "Deals (Actual)",
                    "Planned Enrolments": "Enrolments (Plan)",
                    "MTD/Cohort Enrolments (Actual)": "Enrolments (Actual)",
                }).melt(
                    "JetLearn Deal Source",
                    var_name="MetricType",
                    value_name="Value",
                )
                bars_df["Metric"] = bars_df["MetricType"].str.split(" \\(", expand=True)[0]
                bars_df["Type"] = bars_df["MetricType"].str.extract("\\((.*?)\\)")
                bars = alt.Chart(bars_df.drop(columns=["MetricType"])).mark_bar().encode(
                    x=alt.X("JetLearn Deal Source:N", sort="-y"),
                    y=alt.Y("Value:Q"),
                    color=alt.Color("Type:N"),
                    tooltip=["JetLearn Deal Source", "Metric", "Type", "Value"],
                ).facet(column="Metric:N")

                conv_df = merged[[
                    "JetLearn Deal Source", "Hist. Conv%", "MTD/Cohort Conv% (Actual)"
                ]].copy().rename(columns={
                    "Hist. Conv%": "Conversion % (Plan)",
                    "MTD/Cohort Conv% (Actual)": "Conversion % (Actual)",
                }).melt(
                    "JetLearn Deal Source",
                    var_name="Type",
                    value_name="Value",
                )
                conv = alt.Chart(conv_df).mark_line(point=True).encode(
                    x=alt.X("JetLearn Deal Source:N", sort="-y"),
                    y=alt.Y("Value:Q", title="Conversion %"),
                    color=alt.Color("Type:N"),
                    tooltip=["JetLearn Deal Source", "Type", "Value"],
                )

                st.write("**Combined Comparison: Deals, Enrolments, and Conversion %**")
                st.altair_chart(alt.vconcat(bars, conv).resolve_scale(y='independent'), use_container_width=True)
        except Exception:
            pass


        # --- Bottom Totals: Deals & Enrolments (bars) + Conversion % (line) ---
        try:
            import altair as alt
            ...
            if all(c in merged.columns for c in req_cols):
                total_plan_deals = float(merged["Planned Deals"].sum())
                total_act_deals = float(merged["MTD/Cohort Deals (Actual)"].sum())
                total_plan_enr = float(merged["Planned Enrolments"].sum())
                total_act_enr  = float(merged["MTD/Cohort Enrolments (Actual)"].sum())

                conv_plan = (total_plan_enr / total_plan_deals * 100.0) if total_plan_deals > 0 else 0.0
                conv_act  = (total_act_enr  / total_act_deals  * 100.0) if total_act_deals  > 0 else 0.0

                # Bars for totals (Deals & Enrolments)
                bars_tot_df = pd.DataFrame({
                    "Metric": ["Deals", "Deals", "Enrolments", "Enrolments"],
                    "Type":   ["Plan", "Actual", "Plan", "Actual"],
                    "Value":  [total_plan_deals, total_act_deals, total_plan_enr, total_act_enr],
                })
                bars_tot = (
                    alt.Chart(bars_tot_df)
                    .mark_bar()
                    .encode(
                        x=alt.X("Metric:N"),
                        y=alt.Y("Value:Q", title="Totals"),
                        color=alt.Color("Type:N"),
                        tooltip=["Metric", "Type", "Value"],
                    )
                )

                # Line for Conversion % totals
                conv_tot_df = pd.DataFrame({
                    "Type": ["Plan", "Actual"],
                    "Value": [conv_plan, conv_act],
                })
                conv_tot = (
                    alt.Chart(conv_tot_df)
                    .mark_line(point=True)
                    .encode(
                ...
                st.write("**Overall Totals: Deals & Enrolments vs Conversion %**")
                st.altair_chart(alt.hconcat(bars_tot, conv_tot).resolve_scale(y='independent'), use_container_width=True)
        except Exception:
            pass

        # --- Bottom Comparison: Select Deals / Enrolments / Conversion % (Plan vs Actual) ---
        try:
            import altair as alt
            # Map friendly names to (plan_col, actual_col)
            metric_options = {
                "Deals": ("Planned Deals", "MTD/Cohort Deals (Actual)"),
                "Enrolments": ("Planned Enrolments", "MTD/Cohort Enrolments (Actual)"),
                "Conversion %": ("Hist. Conv%", "MTD/Cohort Conv% (Actual)"),
            }
            metric_choice = st.selectbox(
                "Bottom comparison metric (Plan vs Actual)",
                options=list(metric_options.keys()),
                index=0,
                key="pva_bottom_metric",
            )
            plan_col, act_col = metric_options[metric_choice]
            # Guard against missing columns
            if plan_col in merged.columns and act_col in merged.columns:
                cmp_df = merged[["JetLearn Deal Source", plan_col, act_col]].copy()
                cmp_df = cmp_df.melt(
                    "JetLearn Deal Source",
                    var_name="Type",
                    value_name="Value",
                )
                cmp_df["Type"] = cmp_df["Type"].replace({
                    plan_col: "Plan",
                    act_col: "Actual",
                })
                chart = (
                ...
                st.write("**Bottom Comparison (Plan vs Actual)**")
                st.altair_chart(chart, use_container_width=True)
            else:
                st.info("Selected metric is not available in the current view.")
        except Exception:
            pass
        # Download
        st.download_button(
            "Download Plan vs Actual (merged) CSV",
            data=merged.to_csv(index=False).encode("utf-8"),
            file_name="marketing_plan_vs_actual.csv",
            mime="text/csv",
            key="dl_pva_merged_csv",
        )
    elif plan_df.empty:
        st.info("Add plan rows to see gap & comparison.")
    else:
        st.info("Actuals missing for this mode; adjust filters or check data.")


# --- Router: Marketing → Plan vs Actual ---
try:
    _master_for_pva = master if 'master' in globals() else st.session_state.get('nav_master', '')
    _view_for_pva = view if 'view' in globals() else st.session_state.get('nav_sub', '')
    if _master_for_pva == "Marketing" and _view_for_pva == "Plan vs Actual":
        # Try common df variable names
        _df_any = None
        for _nm in ("df_f", "dff", "df"):
            _cand = globals().get(_nm, None)
            if _cand is not None:
                _df_any = _cand
                break
        _render_marketing_plan_vs_actual(df_f=_df_any)
except Exception as _e:
    try:
        import streamlit as st
